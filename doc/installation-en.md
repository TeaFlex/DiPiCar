...
*IMPORTANT*: We strongly advise you to change the access point password after the hostapd installation. Not modifying it will not break the application but it's recommended to do so for security purpose. To do this, run `sudo nano /etc/hostapd/hostapd.conf` and modify the `wpa_passphrase` field.
